...
export default function Cities(){
    ...
    const [dataCity, setDataCity] = useState<City>(initialCities);
    ...
    const fetchData = async () => {
        setLoading(true);
        try{
            const res = await fetch(`https://sayaserver.onrender.com/api/cities?page=${currentPage}&pageSize=${pageSize}`, {
            method: 'GET',
            headers: {
            ...
            setDataCities(json.response); // ACTUALIZAR EL ESTADO
    ...
    const handleCreateData = async (e: React.FormEvent) => {
        ...
        if(editMode){
            ...
            const res = await fetch('https://sayaserver.onrender.com/api/cities/', {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
                    "x-api-key": "123456",
    ...
    // EDIT DATA
    const handleEditData = async (city: City) => {
        const selectedCity = dataCities.find(c => c.id === city.id)!;
        
        setDataCity(selectedCity);
        onOpen();
    ...
    const handleUpdateData = async () =>{
        const res = await fetch(`https://sayaserver.onrender.com/api/cities/${dataCity.id}`, {
        method: 'PUT',
        headers: {
            "Content-Type": "application/json",
            "x-api-key": "123456",
    ...
    const handleDeleteData = async (id: string) => {
        const res = await fetch(`https://sayaserver.onrender.com/api/cities/${id}`, {
        method: 'DELETE',
        headers: {
            "Content-Type": "application/json",
            "x-api-key": "123456",
    ...
    // SHOW DATA
    const handleShowData = async (city: City) => {
        const selectedCity = dataCities.find(c => c.id === city.id)!;
        ...
        <>
            <Box px={3} py={3}>
                <Flex justifyContent={'space-between'} alignItems={'center'} mt={'40px'}>
                    <Heading as='h3' size='xl' id='Parents' >Ciudades</Heading>
                    <Box>
                        <ButtonGroup>
                            ...
                            <form onSubmit={handleCreateData}>
                                <Stack spacing={4}>    
                                    ...
                                    <FormControl isRequired>
                                        ...
                                        <Input placeholder="Nombre" value={dataCity.name || ""} type='text' readOnly={showMode}  onChange={(e) => setDataCity({ ...dataCity, name: e.target.value })} />
                                    </FormControl>
